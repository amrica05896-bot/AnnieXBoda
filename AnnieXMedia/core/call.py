...
# System: Call Controller (Masterpiece Edition)
# Fixes: Assistant Join, Auto-Create VC, Proper 'call_8' Error Raising
...
class Call:
    ...
    # ==========================================================
    # 🔥 ULTIMATE JOIN LOGIC (Fixed for call_8)
    # ==========================================================
    async def join_call(self, chat_id: int, original_chat_id: int, link: str, video: Union[bool, str] = None, image: Union[bool, str] = None) -> None:
        assistant = await group_assistant(self, chat_id)
        # Getting the Pyrogram Client properly
        ...
        for attempt in range(retries):
            try:
                # 2. Try Standard Join (Auto-Start)
                await self._play_safe(chat_id, stream, force_join=True)
                ...
                # 🔥 KEY FIX: Immediate Error Raise for Permissions
                if isinstance(e, ChatAdminRequired) or "chat_admin_required" in err_str:
                    raise AssistantErr(_["call_8"])

                # 🔥 KEY FIX: If "No Active Call" and we are at last retry (or explicit failure)
                # We try to CREATE manually to be sure, if that fails, we raise call_8
                if isinstance(e, NoActiveGroupCall) or "noactivegroupcall" in err_str:
                    try:
                        if user_client:
                            # Attempt creation to check permission
                            ...
                            # If creation works, wait and retry join
                            await asyncio.sleep(4)
                            continue 
                    except Exception as create_err:
                        # If creation fails, it means we don't have permission -> call_8
                        raise AssistantErr(_["call_8"])
