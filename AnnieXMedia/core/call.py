...
# System: Call Controller (Alexa File-System Structure)
# Optimized for: Speed, Stability, and Crash Prevention
...
from AnnieXMedia.utils.inline.play import stream_markup, telegram_markup
...
# --- [1] The Stream Helper (Titan Flags Included) ---
def dynamic_media_stream(path: str, video: bool = False, ffmpeg_params: str = None) -> MediaStream:
    # إعدادات Titan لمنع التقطيع
    titan_flags = "-threads 4 -ac 2"
    
    # لو الرابط مباشر (HTTP/HTTPS) نضيف إعدادات إعادة الاتصال
    if str(path).startswith("http"):
        titan_flags += " -reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -probesize 10M -analyzeduration 10M"
    ...
    return MediaStream(
        media_path=path,
        audio_parameters=AudioQuality.HIGH, # جودة عالية للصوت
...
class Call:
    ...
    async def join_call(
        ...
        try:
            ...
            raise AssistantErr("<b>No Active VideoChat Found.</b>\nPlease start the VideoChat first.")
        except (NoAudioSourceFound, NoVideoSourceFound):
            raise AssistantErr("<b>Error:</b> Source Not Found.")
        except (ConnectionNotFound, TelegramServerError):
             # Retry logic
             await asyncio.sleep(1)
             try: await assistant.play(chat_id, stream, config=ksk)
             except: raise AssistantErr("<b>Connection Error.</b>\nPlease restart VideoChat.")
        except Exception as e:
            if "already joined" in str(e).lower():
                try: await assistant.change_stream(chat_id, stream)
                except: pass
            else:
                raise AssistantErr(f"<b>Error:</b> {e}")
    ...
    async def play(self, client, chat_id: int) -> None:
        ...
        try:
            ...
            try:
                ...
                # 4. STANDARD / YOUTUBE / TELEGRAM
                else:
                    # Automatic Direct Link Fetch for Youtube (Better Stability)
                    if "youtube" in queued or "youtu.be" in queued:
                         try:
                             # Try getting direct link if it's a raw youtube link
                             pass # Logic handled inside Youtube.py mostly, keeping it simple here
                         except: pass
