...
# Optimized: Python 3.13, Pillow 11.x, 100% Async Threading
# Backend: Fully migrated to yt-dlp (Accurate Views, Time, and Title) - NO youtubesearchpython
...
# 🔥 الدالة دي بتشتغل برا مسار بايثون الأساسي عشان الرسم ميجمدش البوت
def _draw_thumbnail(thumb_path, cache_path, title, views_text, duration_text, is_live):
    ...
    final_views = fix_ar(f"YouTube | {views_text}")
...
async def get_thumb(videoid: str) -> str:
    ...
    # ✅ حماية قوية: لو الـ ID مش بتاع يوتيوب (تيك توك أو فيسبوك مثلاً)، نتخطى البحث ونعمل صورة شيك
    if len(videoid) != 11:
        title, thumbnail, duration_text, views_text = "Social Media Stream", YOUTUBE_IMG_URL, "00:00", "High Quality Audio"
        is_live = False
    else:
        try:
            # 🔥 جلب الداتا بسرعة البرق باستخدام yt-dlp
            cmd = ["yt-dlp", "--dump-json", f"https://www.youtube.com/watch?v={videoid}", "--no-warnings", "--skip-download"]
            ...
            if out:
                ...
                thumbnail = data.get("thumbnail", YOUTUBE_IMG_URL).split("?")[0]
                duration_text = data.get("duration_string", "00:00")
                raw_views = data.get("view_count", 0)
                is_live = data.get("is_live", False)
                
                # تنسيق المشاهدات (مثال: 1.5M بدلاً من 1500000)
                if raw_views >= 1000000:
                    views_text = f"{raw_views/1000000:.1f}M Views"
                elif raw_views >= 1000:
                    views_text = f"{raw_views/1000:.1f}K Views"
                else:
                    views_text = f"{raw_views} Views"
                    
                if is_live: duration_text = "Live"
            else:
                raise ValueError("No Data")
        except Exception as e:
            title, thumbnail, duration_text, views_text = "Unknown Track", YOUTUBE_IMG_URL, "00:00", "Audio Stream"
            is_live = False
    ...
    try:
        # ✅ يتم رسم الصورة في الـ Thread لمنع البوت من التجميد
        await asyncio.to_thread(
            _draw_thumbnail, 
            thumb_path, cache_path, title, views_text, duration_text, is_live
        )
        return cache_path
    except Exception as e:
