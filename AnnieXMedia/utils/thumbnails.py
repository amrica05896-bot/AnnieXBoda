...
# Optimized for Pillow 12.1.0 & Python 3.12+
...
# مكتبة البحث غير المتزامن (تأكد أن المكتبة مثبتة)
...
def trim_to_width(text: str, font: ImageFont.FreeTypeFont, max_w: int) -> str:
    """قص النص بذكاء ليتناسب مع العرض المحدد"""
    ellipsis = "…"
    # Pillow 10+ uses getlength correctly
...
async def get_thumb(videoid: str) -> str:
    ...
    cache_path = os.path.join(CACHE_DIR, f"{videoid}_v6_glass_aio.png")
    if os.path.exists(cache_path):
        return cache_path

    # 1. جلب البيانات من يوتيوب
    try:
        # البحث باستخدام الـ Video ID
        search = VideosSearch(videoid, limit=1)
        results_data = await search.next()
        result_items = results_data.get("result", [])
        
        if not result_items:
            # محاولة أخرى بالرابط المباشر لو الـ ID فشل
    ...
    # ================= معالجة الصورة (The Magic) =================
    try:
        # فتح الصورة وتحويلها لـ RGBA
        base = Image.open(thumb_path).convert("RGBA")
        
        # تغيير الحجم باستخدام LANCZOS (أعلى جودة في Pillow 12)
        base = base.resize((1280, 720), Image.Resampling.LANCZOS)
        
        # 1. الخلفية العامة (Blurry Background)
        bg = base.filter(ImageFilter.BoxBlur(5)) # BoxBlur أسرع وأخف
        bg = ImageEnhance.Brightness(bg).enhance(0.6) # تغميق الخلفية

        # 2. المنطقة الزجاجية (Glass Panel)
        crop = bg.crop((PANEL_X, PANEL_Y, PANEL_X + PANEL_W, PANEL_Y + PANEL_H))
        crop = crop.filter(ImageFilter.GaussianBlur(30)) # تمويه قوي للزجاج
        crop = ImageEnhance.Color(crop).enhance(1.5)      # تشبيع الألوان
        crop = ImageEnhance.Brightness(crop).enhance(1.1) # إضاءة خفيفة
        
        # طبقة التبييض الشفافة
        tint = Image.new("RGBA", crop.size, (255, 255, 255, TRANSPARENCY))
        glass_panel = Image.alpha_composite(crop, tint)
        
        # قناع الزوايا الدائرية للبانل
        ...
        # 3. الصورة المصغرة الداخلية (Album Art)
        # استخدام LANCZOS للتصغير النظيف
        thumb_inner = base.resize((THUMB_W, THUMB_H), Image.Resampling.LANCZOS)
        
        # قناع للصورة الداخلية
        ...
        if os.path.isfile(icons_path):
            ...
            # تحويل الأيقونات للون الأسود لتناسب الخلفية الفاتحة
            r, g, b, a = ic.split()
